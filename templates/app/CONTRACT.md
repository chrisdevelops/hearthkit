...
`appTemplateSupersetEnvVariableNames` is the whole set the superset template reads: those three followed by each optional package's `envVariableNames`, in `appTemplateOptionalPackageNames` order. It is written out by hand rather than derived, so a gate proves the map and the list agree instead of the list being true by construction. **Every name in it was read off the owning package's `envSchemaFragment`, not off plan section 4**, which is stale in two places: it omits `storage`'s `STORAGE_REGION`, and it says "transport credentials" for `email` where there are seven named variables.
...
- **Absent selection means the superset.** Nothing is pruned for being optional. This is what `verify:container` gets, and it is why that command keeps working with no change at its call site.
...
- `pruneOptionalSectionBlocks` removes, for each unselected package, every line from a begin marker through its end marker **inclusive**, plus any blank lines immediately following the end marker. Everything else is returned byte for byte.
- The trailing-blank rule is what makes the result independent of which subset was removed. Each block is written with exactly one blank line after its end marker, so removing any combination leaves one blank line between what remains, and removing all four returns the file to the state it is in today.
...
1. **A generated project contains no marker text at all.** Not one occurrence of `appTemplateSectionBlockBeginPrefix` or `appTemplateSectionBlockEndPrefix` in any file, at any selection including the full superset. A surviving marker means the pruner skipped a file it should have processed, and the project still runs — so nothing else catches it.
...
- **Isolation is mandatory, not a nicety.** `@hearthkit/email`'s own gates clear Mailpit wholesale and count its messages. A template flow sending into the same container collides with them exactly as the `auth` gates would have. The flow must scope every read with `GET /api/v1/search?query=to:"<unique address>"` and **must never call `DELETE /api/v1/messages` or `DELETE /api/v1/search`**. Giving the flows their own Mailpit is the alternative and is equally acceptable.
...
- **Local infrastructure:** Postgres and Mailpit. The same Mailpit isolation rule as the email section applies to reading the magic link.
...
- **`app-template-optional-block-malformed`** — a block-pruned file's markers do not balance. Carries `blockPrunedPath`, `owningOptionalPackageName`, and `blockMarkerProblem`, one of `begin-without-end`, `end-without-begin`, `begin-inside-open-block`, `duplicate-block-for-package`. Prefix `hearthkit app template optional block malformed:`. **This is the guard on the one place a sloppy rule silently produces a broken file**: without it, an unterminated begin marker deletes the rest of the file and the result still parses often enough to be missed.
...
- `app-template-optional-block-malformed` is gated on the template's own files (their markers must balance) and on synthetic text for each of the four `blockMarkerProblem` values, which needs no filesystem.
...
- **Fast, and new with the sections:** the map parses through `appTemplateOptionalPackageSectionSchema`; `ownedTemplatePaths` and `envVariableNames` do not overlap between packages; every owned path exists on disk; every `flowSpecPath` is also an owned path; `appTemplateSupersetEnvVariableNames` equals the always-on three plus the map's lists; every block-pruned file's markers balance; every package has at least one block in each of its `blockPrunedPaths`; every section page and route handler exports `dynamic = 'force-dynamic'`; **materializing with an empty selection reproduces `appGeneratedProjectGuaranteedPaths` exactly and leaves no occurrence of either marker prefix in any file**; and the four `blockMarkerProblem` values against synthetic text.
...
No open questions. Rounds 1 to 3 are closed, and round 4's three questions were ruled on 2026-09-06.
