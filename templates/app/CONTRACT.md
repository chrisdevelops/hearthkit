...
The full set of things `create` must change instead of copying is `appTemplateScaffoldRewriteTargets`. The first five are Phase 4's; the next four only ever delete; the tenth is the one value substitution in source:
...
- `organizations-flag-literal` — when the `organizations` scaffold flag is on, rewrite exactly one line of `app-auth-server.ts`, `export const appOrganizationsEnabled = false`, to `true`. Nothing else in source is ever substituted (ruled 2026-09-07).
...
- **Never copied:** `node_modules`, `.next`, `test-results`, `playwright-report` (`appTemplateNeverCopiedDirectoryNames`), and the loose build output or OS noise `next-env.d.ts`, `tsconfig.tsbuildinfo`, `.DS_Store` wherever they sit (`appTemplateNeverCopiedFileNames`).
...
**Both pruners live in `@hearthkit/create`** (completion plan 3.2); this template ships only the manifest they read, `src/app-template-contract.ts`. `decideTemplatePathPrune` and `pruneOptionalSectionBlocks` are two halves of one rule — one decides whether a file survives, the other decides which of its lines do — and `create`'s gates read both from that package, so neither may be an unexported local the way `isPrunedTemplatePath` was.
...
- **Conditional or templated file content.** No placeholder tokens anywhere. **The scaffolder rewrites JSON fields, deletes files, and deletes marked blocks; it never substitutes a value in source, with exactly one declared exception: `organizations-flag-literal` rewrites the single line `export const appOrganizationsEnabled = false` in `app-auth-server.ts` to `true` (ruled 2026-09-07).** (This replaces the Phase 4 wording, "it never string-substitutes source", which was written before any file needed a marked block and which this contract would otherwise contradict. The prohibition it was guarding is unchanged and now says what it means: JSON fields may be rewritten, source may only be deleted from.) **A marked block is a deletion unit, not a template**, which is why `app/page.tsx` carries no block and links to no section: a home page that referenced a section would need content rewritten rather than removed.
